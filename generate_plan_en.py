...
def main():
    ...
    if actual_weeks < profile_weeks:
        print(f"  Weeks:     {actual_weeks} (shortened from {profile_weeks} — not enough time)")
    elif actual_weeks > profile_weeks:
        print(f"  Weeks:     {actual_weeks} (plan starts today; profile: {profile_weeks})")
    else:
    ...
    if args.strength:
        ...
        print(f"  + {len(strength_wkts)} strength/mobility sessions (--strength)")
