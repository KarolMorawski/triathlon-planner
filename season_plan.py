...
# (single source of truth, imported above).
...
def generate_race_block(race_date, distance, ftp, run_pace_ms, prefix,
    ...
    for wk in range(1, weeks + 1):
        ...
        if is_build and long_run_day != 5:
            ...
            steps = [run_wu(1, 300), run_int(2, brick_km * 1000, race_p * 0.97, race_p * 1.03), run_cd(3, 300)]
            workouts.append((_wkt("run", f"{tag} Brick Run {brick_km}km @{ms_to_pace(race_p)}/km",
                "Brick: off-the-bike transition run at race pace", steps, (brick_km + 1) * 1000), D(5)))
...
def clean_prefix(client, prefix):
    ...
    removed_s, failed_s = clean_calendar_prefix(client, prefix, sleep_s=0.08)
    ...
    removed_l, failed_l = clean_library_prefix(client, prefix, sleep_s=0.08)
...
def interactive_config():
    ...
    lrd_in = input("Long run day — 6=Niedziela (domyślnie), 5=Sobota: ").strip()
    long_run_day = 5 if lrd_in in ("5", "sob", "sobota", "sat", "saturday") else 6
...
def main():
    ...
    lrd_name = "Sobota" if long_run_day == 5 else "Niedziela"
    ...
    for race in sorted(races, key=lambda r: r["date"]):
        ...
        if use_bridge:
            ...
            print(f"    Block: {start} → {race['date']}  ({block_weeks}w — profil: {full_weeks}w + {extra}w bazy)")
        elif use_truncated:
            reason = "za mało czasu" if first_race else "gap od poprzedniego wyścigu"
